...
class Fabric_host(object):
    ...
    def add_host(self):
        while True:
            ...
            hostname = input("请输入远程主机名(q退出):").strip()
            if hostname == 'q':
                continue
            ip = input("请输入远程主机%s的ip(q退出)："%hostname).strip()
            if ip == 'q':
                break
            user = input("请输入主机%s用户名(q退出):"%hostname).strip()
            if user == 'q':
                break
            passwd = input("请输入用户%s的登录密码(q退出):"%user).strip()
            if passwd == 'q':
                break
            port = input("请输入主机%s的ssh端口(q退出):"%hostname).strip()
            if port == 'q':
                break
