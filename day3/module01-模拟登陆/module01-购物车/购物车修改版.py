...
shopping_record_line = shopping_record_file.read()  #读取购物记录文件
...
if username in User_info_line.keys() and password == User_info_line["{user}".format(user=username)] and username not in shopping_record_line:
    ...
    while True:
        ...
        if choice.isdigit():
            ...
            shopping_record_file.write('你的账户{user}余额:'.format(user=username) + str(salary) + '\n')  # 把账户和余额写入购物清单
            shopping_record_file.write('你的购物单:'.format(user=username) + str(shopping_list) + '\n')  # 把购物单写入购物清单
            print("------shopping list-----")
            print(shopping_record_line)
            shopping_record_file.close()
            exit()
